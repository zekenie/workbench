...
const nodeState: NodeState = {
  node1: `
    const apiData = await (await fetch("/some-api")).json();
    const processedData = apiData.map(item => item.id);
    export { apiData, processedData };
  `,
  node2: `
    import { processedData } from 'node1';
    const doubledIds = processedData.map(id => id * 2);
    export const result = {
  ...
  `,
  node3: `
    import { apiData } from 'node1';
    import { result as doubledResult } from 'node2';
    export default {
      raw: apiData,
      processed: doubledResult
    };
  `,
};
...
const generator = new ExplicitExportDependencyTypeGenerator(
  nodeState,
  dependencyState
);
const declarationFile = generator.generateDependencyTypes("node3");
